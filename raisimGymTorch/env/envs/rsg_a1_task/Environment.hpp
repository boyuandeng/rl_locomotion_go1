...
namespace raisim
{
  ...
  public:
    explicit ENVIRONMENT(const std::string &resourceDir, const Yaml::Node &cfg, bool visualizable, int env_id) : RaisimGymEnv(resourceDir, cfg), visualizable_(visualizable)
    {
      ...
      a1_ = world_->addArticulatedSystem(resourceDir_ + "/go1/urdf/go1.urdf");
      a1_->setName("go1");
      ...
      jt_mean_pos << 0.05, 0.8, -1.4, -0.05, 0.8, -1.4, 0.05, 0.8, -1.4, -0.05, 0.8, -1.4;//a1
      // jt_mean_pos << 0.1, 0.72, -1.44, -0.1, 0.72, -1.44, 0.1, 0.72, -1.44, -0.1, 0.72, -1.44;//go1
      // randomize terrains
      randomize_terrain();
